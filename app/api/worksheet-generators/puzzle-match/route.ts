...
import { newWorksheetDoc, addThemedWorksheetPage, loadBundleTheme, drawThemeBorder, uploadWorksheetPdf, shuffle, randInt, PAGE_W, PAGE_H, INK, NAVY, asciiSafeFilename} from '@/lib/worksheet-pdf';
...
export async function POST(request: NextRequest) {
  try {
    const { userId, operation = 'multiplication', count = 16, title, bundleId } = (await request.json()) || {};
    ...
    const drawCards = async (items: { text: string }[], heading: string) => {
      let page: any = null;
      for (let i = 0; i < items.length; i++) {
        const item = items[i];
        const pos = i % (cols * rows);
        if (pos === 0) { page = doc.addPage([PAGE_W, PAGE_H]); await drawThemeBorder(doc, page, theme); page.drawText(heading, { x: margin, y: PAGE_H - 24, size: 12, font: helvBold, color: NAVY }); }
    ...
    await drawCards(problems.map((p) => ({ text: p.text })), `${docTitle} -- Problem Cards (cut apart)`);
    await drawCards(shuffle(problems.map((p) => ({ text: String(p.answer) }))), `${docTitle} -- Answer Cards (cut apart)`);
