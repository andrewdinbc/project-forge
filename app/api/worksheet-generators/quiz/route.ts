...
import { newWorksheetDoc, addWorksheetPage, uploadWorksheetPdf, wrapLines, shuffle, PAGE_W, PAGE_H, INK, NAVY, LINE } from '@/lib/worksheet-pdf';
...
export async function POST(request: NextRequest) {
  try {
    const { userId, type, content, title } = (await request.json()) || {};
    ...
    if (type === 'multiple_choice') {
      ...
      questions.forEach((q, i) => {
        if (y < 120) { page = doc.addPage([PAGE_W, PAGE_H]); y = PAGE_H - 60; }
      ...
      questions.forEach((q: string, i: number) => {
        if (y < 140) { page = doc.addPage([PAGE_W, PAGE_H]); y = PAGE_H - 60; }
