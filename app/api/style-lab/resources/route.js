...
export async function POST(request) {
  ...
  try {
    ...
    // "Add to Visual Layer Processing" (Aj, 2026-07-19): a lightweight staging
    // flag separating "bulk-imported for reference/inspiration" from "actively
    // considering this one" -- distinct from and upstream of actually saving
    // anything to Parts Library, which remains the only place content becomes
    // reusable. Toggle, not one-way, so a resource can be un-flagged again.
    if (action === 'toggle_processing') {
      const { marked } = body;
      const row = await updateResource(userId, id, { marked_for_processing: !!marked });
      return NextResponse.json({ ok: true, resource: row });
    }

    // Removed 2026-07-19 per Aj: "I want nothing going directly into AI
    // steering. Instead I want my products I have made go into AI steering."
    // Imported/purchased Style Lab reference material -- even a teacher-
    // edited or AI-cleaned version of it -- should never feed AI Steering,
    // which influences every future generation across the app. This closes
    // the actual capability, not just the UI button (already removed
    // earlier); see app/api/products/[productId]/push-to-steering for the
    // replacement, scoped to Aj's own authored products only.
    if (action === 'push_to_steering') {
      return NextResponse.json(
        { error: 'Style Lab resources can no longer push to AI Steering directly. Push your own products to AI Steering from the Dashboard product page instead.' },
        { status: 410 }
      );
    }
