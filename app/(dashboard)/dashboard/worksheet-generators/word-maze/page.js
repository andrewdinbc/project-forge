...
export default function WordMazePage() {
  ...
  async function generate() {
    ...
    try {
      const res = await fetch('/api/worksheet-generators/word-maze', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId, size, fact: fact || undefined, title: title || undefined, bundleId: bundleId || undefined }),
      })
      ...
      const a = document.createElement('a'); a.href = url; a.download = 'word-maze.pdf'; a.click()
  ...
  return (
    <div style={{ fontFamily: FONT_BODY, maxWidth: 700, margin: '0 auto', padding: '24px 20px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8, marginBottom: 4 }}>
        <h1 style={{ color: C.navy, fontSize: 22, margin: 0 }}>🌀 Word Maze</h1>
        <a href="/dashboard/worksheet-generators" style={{ fontSize: 12, color: '#888' }}>← All Generators</a>
      </div>
      <p style={{ fontSize: 13, color: '#666', marginBottom: 20 }}>
        A real maze (recursive-backtracker generated, guaranteed solvable, exactly one path from
        start to end) -- the letters along the true path spell out a secret fact. Answer key included.
      </p>

      <div style={{ background: '#fff', border: `1px solid ${C.border}`, borderRadius: 8, padding: 16 }}>
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 12, flexWrap: 'wrap' }}>
          <label style={{ fontSize: 12, color: '#555' }}>Maze size:</label>
          <select value={size} onChange={(e) => setSize(e.target.value)} style={{ fontSize: 12, padding: '5px 8px', border: `1px solid ${C.border}`, borderRadius: 5 }}>
            <option value="small">Small (8x8, younger grades)</option>
            <option value="medium">Medium (11x11)</option>
            <option value="large">Large (14x14, older grades)</option>
          </select>
        </div>

        <div style={{ marginBottom: 12 }}>
          <label style={{ fontSize: 12, color: '#555', display: 'block', marginBottom: 4 }}>Secret fact (optional -- leave blank for a random one):</label>
          <input
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          >
            {generating ? 'Generating…' : '📄 Generate Word Maze'}
          </button>
        </div>
        {msg && <p style={{ fontSize: 12, color: msg.startsWith('✓') ? '#2f6b41' : '#a33', marginTop: 8 }}>{msg}</p>}
        <SaveAsProductBar userId={userId} fileUrl={fileUrl} defaultTitle={title || 'Word Maze'} />
      </div>
    </div>
  )
}
