...
export default function ColorByNumberPage() {
  ...
  async function generate() {
    ...
    try {
      const res = await fetch('/api/worksheet-generators/color-by-number', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          userId, prompt: prompt.trim(), complexity,
          title: title || `Color by Number: ${prompt.trim()}`, bundleId: bundleId || undefined,
        }),
      })
      ...
      const a = document.createElement('a'); a.href = url; a.download = 'color-by-number.pdf'; a.click()
  ...
  return (
    <div style={{ fontFamily: FONT_BODY, maxWidth: 700, margin: '0 auto', padding: '24px 20px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8, marginBottom: 4 }}>
        <h1 style={{ color: C.navy, fontSize: 22, margin: 0 }}>🎨 Color by Number</h1>
        <a href="/dashboard/worksheet-generators" style={{ fontSize: 12, color: '#888' }}>← All Generators</a>
      </div>
      <p style={{ fontSize: 13, color: '#666', marginBottom: 20 }}>
        Describe any subject and this AI-generates a fresh flat-color picture, then pixelates it into
        a numbered grid with a color key -- the same mystery-picture format Super Teacher Worksheets
        uses, but any theme you ask for instead of picking from a fixed library. Comes with a colored
        answer key. Needs GEMINI_API_KEY set in this project's environment variables.
      </p>
      ...
      <div style={{ background: '#fff', border: `1px solid ${C.border}`, borderRadius: 8, padding: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 12, flexWrap: 'wrap' }}>
          ...
          <select value={complexity} onChange={(e) => setComplexity(e.target.value)} style={{ fontSize: 12, padding: '5px 8px', border: `1px solid ${C.border}`, borderRadius: 5 }}>
            <option value="simple">Simple (14x18 -- younger grades)</option>
            <option value="detailed">Detailed (22x28 -- older grades)</option>
          </select>
        </div>
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          >
            {generating ? 'Generating… (can take ~20s)' : '📄 Generate Color by Number'}
          </button>
        </div>
        {msg && <p style={{ fontSize: 12, color: msg.startsWith('✓') ? '#2f6b41' : '#a33', marginTop: 8 }}>{msg}</p>}
        <SaveAsProductBar userId={userId} fileUrl={fileUrl} defaultTitle={title || `Color by Number: ${prompt}`} />
      </div>
    </div>
  )
}
