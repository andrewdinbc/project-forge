...
export default function WordSearchPage() {
  ...
  async function generate() {
    ...
    try {
      const res = await fetch('/api/worksheet-generators/word-search', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId, words, difficulty, title: title || 'Word Search' }),
      })
      ...
      const a = document.createElement('a'); a.href = url; a.download = 'word-search.pdf'; a.click()
  ...
  return (
    <div style={{ fontFamily: FONT_BODY, maxWidth: 700, margin: '0 auto', padding: '24px 20px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8, marginBottom: 4 }}>
        <h1 style={{ color: C.navy, fontSize: 22, margin: 0 }}>🔎 Word Search Maker</h1>
        <a href="/dashboard/worksheet-generators" style={{ fontSize: 12, color: '#888' }}>← All Generators</a>
      </div>
      <p style={{ fontSize: 13, color: '#666', marginBottom: 20 }}>
        One word per line. Basic = across/down only. Intermediate adds diagonals. Advanced adds
        backwards too. A highlighted answer key is included. If a word can't fit, it's listed at the
        bottom of the puzzle instead of silently vanishing.
      </p>
      ...
      <div style={{ background: '#fff', border: `1px solid ${C.border}`, borderRadius: 8, padding: 16 }}>
        ...
        <SaveAsProductBar userId={userId} fileUrl={fileUrl} defaultTitle={title || 'Word Search'} />
      </div>
    </div>
  )
}
