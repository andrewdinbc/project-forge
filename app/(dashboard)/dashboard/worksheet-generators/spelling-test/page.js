...
export default function SpellingTestPage() {
  ...
  async function generate() {
    ...
    try {
      const res = await fetch('/api/worksheet-generators/spelling-test', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId, words, mode, title: title || undefined, bundleId: bundleId || undefined }),
      })
      ...
      const a = document.createElement('a'); a.href = url; a.download = 'spelling-test.pdf'; a.click()
  ...
  return (
    <div style={{ fontFamily: FONT_BODY, maxWidth: 700, margin: '0 auto', padding: '24px 20px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8, marginBottom: 4 }}>
        <h1 style={{ color: C.navy, fontSize: 22, margin: 0 }}>📝 Spelling Test / Practice Sheet</h1>
        <a href="/dashboard/worksheet-generators" style={{ fontSize: 12, color: '#888' }}>← All Generators</a>
      </div>
      <p style={{ fontSize: 13, color: '#666', marginBottom: 20 }}>
        A real dictation test page (numbered blank lines, read the words aloud) and/or the classic
        Look-Say-Cover-Write-Check independent practice table. Paste a list, or generate one first
        with the Spelling List Generator and send it here.
      </p>
      ...
      <div style={{ background: '#fff', border: `1px solid ${C.border}`, borderRadius: 8, padding: 16 }}>
        ...
        />
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginTop: 10, marginBottom: 12, flexWrap: 'wrap' }}>
          <label style={{ fontSize: 12, color: '#555' }}>Include:</label>
          <select value={mode} onChange={(e) => setMode(e.target.value)} style={{ fontSize: 12, padding: '5px 8px', border: `1px solid ${C.border}`, borderRadius: 5 }}>
            <option value="both">Test + Practice sheet</option>
            <option value="test">Test only (dictation)</option>
            <option value="practice">Practice only (Look-Say-Cover-Write-Check)</option>
          </select>
        </div>
        ...
        <SaveAsProductBar userId={userId} fileUrl={fileUrl} defaultTitle={title || 'Spelling Test'} />
      </div>
    </div>
  )
}
