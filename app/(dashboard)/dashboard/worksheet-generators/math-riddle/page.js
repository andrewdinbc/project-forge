...
export default function MathRiddlePage() {
  ...
  async function generate() {
    ...
    try {
      const res = await fetch('/api/worksheet-generators/math-riddle', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId, topic, grade, title: title || 'Math Riddle' }),
      })
      ...
      const a = document.createElement('a'); a.href = url; a.download = 'math-riddle.pdf'; a.click()
  ...
  return (
    <div style={{ fontFamily: FONT_BODY, maxWidth: 700, margin: '0 auto', padding: '24px 20px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8, marginBottom: 4 }}>
        <h1 style={{ color: C.navy, fontSize: 22, margin: 0 }}>😄 Math Riddle Worksheet</h1>
        <a href="/dashboard/worksheet-generators" style={{ fontSize: 12, color: '#888' }}>← All Generators</a>
      </div>
      <p style={{ fontSize: 13, color: '#666', marginBottom: 20 }}>
        AI writes an original joke; solving each math problem and decoding the number key reveals the punchline.
      </p>

      <div style={{ background: '#fff', border: `1px solid ${C.border}`, borderRadius: 8, padding: 16 }}>
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 12, flexWrap: 'wrap' }}>
          <input value={topic} onChange={(e) => setTopic(e.target.value)} placeholder="Theme (optional, e.g. dinosaurs)" style={{ flex: 1, minWidth: 200, fontSize: 12, padding: '6px 8px', border: `1px solid ${C.border}`, borderRadius: 5 }} />
        ...
        <SaveAsProductBar userId={userId} fileUrl={fileUrl} defaultTitle={title || 'Math Riddle'} />
      </div>
    </div>
  )
}
