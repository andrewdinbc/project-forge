...
export default function CryptogramPage() {
  ...
  async function generate() {
    ...
    try {
      const res = await fetch('/api/worksheet-generators/cryptogram', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId, phrase, title: title || 'Cryptogram' }),
      })
      ...
      const a = document.createElement('a'); a.href = url; a.download = 'cryptogram.pdf'; a.click()
  ...
  return (
    <div style={{ fontFamily: FONT_BODY, maxWidth: 700, margin: '0 auto', padding: '24px 20px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8, marginBottom: 4 }}>
        <h1 style={{ color: C.navy, fontSize: 22, margin: 0 }}>🔐 Cryptogram Generator</h1>
        <a href="/dashboard/worksheet-generators" style={{ fontSize: 12, color: '#888' }}>← All Generators</a>
      </div>
      <p style={{ fontSize: 13, color: '#666', marginBottom: 20 }}>
        Type a phrase and it turns into a number-substitution puzzle to decode, with a numbered legend and an answer key.
      </p>

      <div style={{ background: '#fff', border: `1px solid ${C.border}`, borderRadius: 8, padding: 16 }}>
        <textarea
          value={phrase} onChange={(e) => setPhrase(e.target.value)} rows={4}
          placeholder="The early bird catches the worm."
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginTop: 10, flexWrap: 'wrap' }}>
          ...
          <button
            onClick={generate} disabled={generating || !userId || !phrase.trim()}
        ...
        <SaveAsProductBar userId={userId} fileUrl={fileUrl} defaultTitle={title || 'Cryptogram'} />
      </div>
    </div>
  )
}
