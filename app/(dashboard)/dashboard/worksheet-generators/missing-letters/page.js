...
export default function MissingLettersPage() {
  ...
  async function generate() {
    ...
    try {
      const res = await fetch('/api/worksheet-generators/missing-letters', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId, words, title: title || 'Missing Letters' }),
      })
      ...
      const a = document.createElement('a'); a.href = url; a.download = 'missing-letters.pdf'; a.click()
  ...
  return (
    <div style={{ fontFamily: FONT_BODY, maxWidth: 700, margin: '0 auto', padding: '24px 20px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8, marginBottom: 4 }}>
        <h1 style={{ color: C.navy, fontSize: 22, margin: 0 }}>🔡 Missing Letters</h1>
        <a href="/dashboard/worksheet-generators" style={{ fontSize: 12, color: '#888' }}>← All Generators</a>
      </div>
      <p style={{ fontSize: 13, color: '#666', marginBottom: 20 }}>
        One spelling/vocabulary word per line. Produces a fill-in-the-blank worksheet with a word bank, plus an answer key.
      </p>
      ...
      <div style={{ background: '#fff', border: `1px solid ${C.border}`, borderRadius: 8, padding: 16 }}>
        ...
        <SaveAsProductBar userId={userId} fileUrl={fileUrl} defaultTitle={title || 'Missing Letters'} />
      </div>
    </div>
  )
}
