...
// Sanitizes a title for safe use in an HTTP Content-Disposition filename
// (2026-07-21). Header values must be a ByteString -- every char code
// <=255 -- and typographic punctuation common in AI-generated or
// auto-planned titles (em dash \u2014, en dash, curly quotes) breaks
// that, throwing "Cannot convert argument to a ByteString" at the
// Response layer. This is a distinct failure mode from the WinAnsi
// PDF-glyph encoding issues already fixed above in this file (those are
// about what pdf-lib can draw on the page; this is about what a raw HTTP
// header can carry) -- found via the bundle auto-generator's own
// generated titles, which use em dashes throughout.
export function asciiSafeFilename(title) {
  const ascii = String(title || 'worksheet')
    .normalize('NFKD')
    .replace(/[\u2010-\u2015]/g, '-')
    .replace(/[\u2018\u2019]/g, "'")
    .replace(/[\u201C\u201D]/g, '"')
    ...
    .replace(/\s+/g, '-')
    .trim();
  return ascii || 'worksheet';
}

// Sanitizes AI-generated text before it reaches pdf-lib's drawText, which
// throws hard on any character outside the WinAnsi encoding (2026-07-21,
// found via the Interactive Notebook generator crashing on an AI-written
// arrow character "\u2190"). Common symbols an AI is likely to reach for
// (arrows, checkmarks, math comparison operators) get a readable ASCII
// replacement; anything else outside WinAnsi's real range gets stripped
// as a safe fallback rather than crashing the whole generation. Preserves
// real WinAnsi characters (em/en dash, curly quotes, ellipsis, bullet) --
// this is a different, narrower problem than asciiSafeFilename above,
// which strips ALL non-ASCII for an HTTP header's stricter ByteString
// requirement.
...
export function sanitizeForPdfText(text) {
  ...
  for (const [char, replacement] of Object.entries(PDF_TEXT_REPLACEMENTS)) {
    if (result.includes(char)) result = result.split(char).join(replacement);
  }
  // Safe fallback: strip anything remaining outside ASCII + the Latin-1
  // supplement + the handful of WinAnsi-safe typographic characters,
  // rather than letting an unanticipated symbol crash the whole PDF.
  return result.replace(/[^\x00-\x7E\xA0-\xFF\u2018\u2019\u201C\u201D\u2013\u2014\u2026\u2022]/g, '');
}

// Recursively sanitizes every string value in a parsed AI JSON response
// (title, component content/labels/contents/outline, etc.) in one pass,
// so every downstream drawText call gets clean text automatically --
// one choke point instead of patching each call site.
export function sanitizeAiJsonText(value) {
  if (typeof value === 'string') return sanitizeForPdfText(value);
  if (Array.isArray(value)) return value.map(sanitizeAiJsonText);
  if (value && typeof value === 'object') {
    const out = {};
    for (const [k, v] of Object.entries(value)) out[k] = sanitizeAiJsonText(v);
    return out;
  }
  return value;
}
