...
export function planUnitResources(unit) {
  ...
  // The catalog has no single 'math' key -- the Math Generators group
  // lists 8 per-operation keys (addition-basic, division-advanced, etc.)
  // that all share the one /api/worksheet-generators/math route. Checking
  // isLive('math') was always false (undefined catalog entry), so no
  // Mathematics unit ever actually produced a job -- found 2026-07-20 via
  // a real end-to-end test once the RLS bug was out of the way. Using
  // 'addition-basic' as a live-status proxy for the shared route.
  if (wantsMath && isLive('addition-basic')) {
