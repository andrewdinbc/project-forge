...
import { addWorksheetPage, wrapLines, PAGE_W, PAGE_H, INK, NAVY, LINE } from './worksheet-pdf';
...
export function drawBoxesLayout({ doc, helv, helvBold, title, subtitle }, slots, content) {
  let page = addWorksheetPage(doc, helvBold, helv, title, subtitle);
  const top = PAGE_H - 140, boxW = PAGE_W - 108;
  const boxH = Math.min(90, (top - 60) / slots.length);
  let y = top;
  slots.forEach((label) => {
    if (y - boxH < 50) { page = doc.addPage([PAGE_W, PAGE_H]); y = PAGE_H - 60; }
...
export function drawColumnsLayout({ doc, helv, helvBold, title, subtitle }, columns, content) {
  const page = addWorksheetPage(doc, helvBold, helv, title, subtitle);
...
export function drawRadialLayout({ doc, helv, helvBold, title, subtitle }, centerLabel, slots, content) {
  const page = addWorksheetPage(doc, helvBold, helv, title, subtitle);
...
export function drawVennLayout({ doc, helv, helvBold, title, subtitle }, labelA, labelB, labelBoth, content) {
  const page = addWorksheetPage(doc, helvBold, helv, title, subtitle);
...
export function drawChainLayout({ doc, helv, helvBold, title, subtitle }, count, content) {
  const page = addWorksheetPage(doc, helvBold, helv, title, subtitle);
...
export function drawQuadrantLayout({ doc, helv, helvBold, title, subtitle }, centerLabel, slots, content) {
  const page = addWorksheetPage(doc, helvBold, helv, title, subtitle);
...
export function drawTreeLayout({ doc, helv, helvBold, title, subtitle }, rootLabel, levels, content) {
  const page = addWorksheetPage(doc, helvBold, helv, title, subtitle);
