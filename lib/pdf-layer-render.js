...
export async function renderPageWithLayers(pdfBytes, pageNumber, layers, scale = 1.3) {
  ...
  // Edit the target page's content stream IN PLACE in the original doc. This
  // keeps every original font/XObject object intact -- copying the page into a
  // fresh doc (copyPages) breaks embedded-font linkage and forces pdf.js into
  // per-glyph path generation, which times out on text-heavy real pages.
